...
def get_random_string(length=20, chars=string.ascii_lowercase + string.ascii_uppercase + string.digits):
...
class BaseConsumer(django_models.Model):
    ...
    content_type = django_models.ForeignKey(ContentType, on_delete=django_models.CASCADE, null=True)
    ...
    TYPES = (
        ...
        (TYPE_READ_WRITE, "Read/Write"),
    )
    type = django_models.CharField(max_length=2, choices=TYPES, default=TYPE_READ_WRITE, db_index=True)
...
class OauthConsumerManager(django_models.Manager):
    def create(self, **kwargs):
        ...
        return super(OauthConsumerManager, self).create(key=key, secret=secret, **kwargs)
